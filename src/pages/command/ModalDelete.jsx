...
export default function ModalDelete({_delete, setDelete, ...props}) {
    ...
    const hanldeDelete = async () => {
      ...
      try {
        const result = await axios.delete(api + `commands/${props.detail.id_command}`, {data: props.detail})
    ...
    return (
      <div>
        ...
        >
          <Box sx={{
            ...
            <Typography id="modal-modal-title" variant="h5" component="h2" sx={{mb: '20px'}}>
              Supprimer {props.detail['command n=°']} !!
            </Typography>
            <Box sx={button_box}>
        ...
        {supprimer && <Notification name={`${props.detail['command n=°']} a été supprimée'`}/>}
      </div>
    );
  }