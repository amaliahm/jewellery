...
export default function ModalDelete({_delete, setDelete, ...props}) {
    ...
    const functionDelete = async (id) => {
      try {
        const result = await axios.delete(api + `magasins`, {data: { id: id}})
    ...
    const hanldeDelete = async () => {
        ...
        Object.keys(props.all_data).map((e, i) => {
            if (props.all_data[e].id_magasin !== props.detail.id_magasin) {
                my_data.push(props.all_data[e])
            }
        })
        my_data.unshift({
            ...props.detail,
            deleted_magasin: true,
        })
      ...
      await functionDelete(props.detail.id_magasin)
    }
    ...
    return (
      <div>
        ...
        >
          <Box sx={{
            ...
            <Typography id="modal-modal-title" variant="h5" component="h2" sx={{mb: '20px'}}>
              Supprimer {props.detail.nom_magasin} !!
            </Typography>
            <Box sx={button_box}>
        ...
        {supprimer && <Notification name={`${props.detail.nom_magasin} a été supprimée'`}/>}
      </div>
    );
  }