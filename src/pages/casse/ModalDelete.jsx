...
export default function ModalDelete({_delete, setDelete, ...props}) {
    ...
    const hanldeDelete = async () => {
        const data = {id_casse: props.detail.id_casse}
      console.log(data)
      setSupprimer(true)
      setTimeout(() => {
        setDelete(false)
      ...
      try {
        const result = await axios.delete(api + `casse/${props.detail.id_casse}`, {data: data})
    ...
    return (
      <div>
        ...
        >
          <Box sx={{
            ...
            <Typography id="modal-modal-title" variant="h5" component="h2" sx={{mb: '20px'}}>
              Supprimer {props.detail['casse n=°']} !!
            </Typography>
            <Box sx={button_box}>
        ...
        {supprimer && <Notification name={`${props.detail['casse n=°']} a été supprimée'`}/>}
      </div>
    );
  }