...
const generateHtmlTable = (data, name, date, nom, id, wilaya, ville, adresse, telephone, total, quantite, vente_numero) => {
  ...
  const signature = `<h6 style="${signature_style}"> EURL BN ZAHAV </h6>`
    ...
    <div style="${info_div_style}">
      <div style="${info_half_div_style}">
        ...
        <div style="${info_head_half_div_style}">
          ...
          <p style="${reste_entreprise_nom_style}">${vente_numero}</p>
          <p style="${reste_entreprise_nom_style}">${date} </p>
        </div>
      </div>
    ...
    <div style="${last_div_style}">
       <p style="${entreprise_nom_style}">total or: </p>
       <p style="${entreprise_nom_style}">${quantite}</p>
    </div>
    <div style="${last_div_style}">
       <p style="${entreprise_nom_style}">total argent: </p>
  ...
  const exportAllDataToPdf = (data, name) => {
    ...
    const id = data[data.length - 1].id_total_vente
    ...
    const vente_numero = data[data.length - 1].vente_numero
    let total = 0
    let quantite = 0
    Object.keys(data).map((e, i) => {
        ...
        quantite += parseFloat(data[e].quantite)
    })
    const htmlContent = generateHtmlTable(all_data, name, date, nom, id, wilaya, ville, adresse, telephone, total, quantite, vente_numero);
