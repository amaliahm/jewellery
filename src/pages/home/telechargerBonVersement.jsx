...
const generateHtmlTable = (data, name, date, nom, id, wilaya, ville, adresse, telephone) => {
  ...
  const signature = `<h6 style="${signature_style}"> EURL BN ZAHAV </h6>`
    ...
    <div style="${info_div_style}">
      <div style="${info_half_div_style}">
        ...
        <div style="${info_head_half_div_style}">
          ...
          <p style="${reste_entreprise_nom_style}">${data.type === 'client' ? data['versement client n=°'] : data['versement fournisseur n=°']}</p>
          <p style="${reste_entreprise_nom_style}">${date} </p>
        </div>
      </div>
    ...
    <div style="${tableau_div_style}">
      <table style="${tableStyles}" border="1">
        <thead>
          <th style="${thStyles}">versement or</th>
          <th style="${thStyles}">versement argent</th>
        </thead>
        <tbody> 
           <tr>
              <td style="${tdStyles}">${data['versement or']}</td>
              <td style="${tdStyles}">${data['versement argent']}$</td>
           </tr>
        </tbody>
      </table>
    </div>
    <div style="${last_div_style}">
       ...
       <p style="${entreprise_nom_style}">${data['ancien solde']}$</p>
    </div>
    <div style="${last_div_style}">
       <p style="${entreprise_nom_style}">nouveau solde: </p>
       <p style="${entreprise_nom_style}">${data['nouveau solde']}$</p>
    </div>
    ${signature}
    </body>
    </html>
  ...
  const exportAllDataToPdf = (data, name) => {
    const htmlContent = generateHtmlTable(data, name, data.date, data.nom, data.id, data.wilaya, data.ville, data.adresse, data.telephone);
