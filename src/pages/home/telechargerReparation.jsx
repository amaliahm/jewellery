...
const generateHtmlTable = (data, name, date, nom, id, wilaya, ville, adresse, telephone, titre, prix) => {
  ...
  const signature = `<h6 style="${signature_style}"> EURL BN ZAHAV </h6>`
    ...
    <div style="${info_div_style}">
      <div style="${info_half_div_style}">
        ...
        <div style="${info_head_half_div_style}">
          ...
          <p style="${reste_entreprise_nom_style}">${data['reparation n=°']}</p>
          <p style="${reste_entreprise_nom_style}">${date} </p>
        </div>
      </div>
      <div style="${info_other_half_div_style}">
        ...
        ${adresse !== null && <p style="${reste_entreprise_nom_style}">{adresse}</p>}
    ...
    <div style="${tableau_div_style}">
      <table style="${tableStyles}" border="1">
        <thead>
          <th style="${thStyles}">magasin</th>
          <th style="${thStyles}">titre</th>
          <th style="${thStyles}">prix</th>
          <th style="${thStyles}">poids</th>
          <th style="${thStyles}">observation</th>
        </thead>
        <tbody> 
           <tr>
              <td style="${tdStyles}">${data.nom_magasin}</td>
              <td style="${tdStyles}">${titre}</td>
              <td style="${tdStyles}">${prix}$</td>
              <td style="${tdStyles}">${data.poids}</td>
              <td style="${tdStyles}">${data.observation}</td>
           </tr>
        </tbody>
      </table>
    </div>
  ...
  const exportAllDataToPdf = (data, name) => {
    const htmlContent_client = generateHtmlTable(data, name, data.date, data.client, data.id_client, data.wilaya_client, data.ville_client, data.adresse_client, data.telephone_client, data.client_titre, data.prix_client);
    const htmlContent_fournisseur = generateHtmlTable(data, name, data.date, data.fournisseur, data.id_fournisseur, data.wilaya_fournisseur, data.ville_fournisseur, data.adresse_fournisseur, data.telephone_fournisseur, data.fournisseur_titre, data.prix_fournisseur);
    exportHtmlToPdf(htmlContent_client);
    exportHtmlToPdf(htmlContent_fournisseur);
  };
