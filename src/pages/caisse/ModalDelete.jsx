...
export default function ModalDelete({_delete, setDelete, ...props}) {
    ...
    const hanldeDelete = async () => {
        const data = {id_caisse: props.detail.id_caisse}
      console.log(data)
      setSupprimer(true)
      setTimeout(() => {
        setDelete(false)
      ...
      try {
        const result = await axios.delete(api + `caisse/${props.detail.id_caisse}`, {data: data})
    ...
    return (
      <div>
        ...
        {supprimer && <Notification name={`cette operation a été supprimée'`}/>}
      </div>
    );
  }